...
test('published Helper completes ready and shutdown handshakes', async () => {
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      output.on('line', (line) => {
        if (line === '{"version":16,"kind":"ready"}' && !shutdownSent) {
          shutdownSent = true
          child.stdin.write('{"version":16,"kind":"shutdown"}\n')
        }
        if (line === '{"version":16,"kind":"closed"}') {
...
test('published Helper remains alive after a valid outputting active state', async () => {
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      const output = createInterface({ input: child.stdout })
      output.on('line', (line) => {
        if (line === '{"version":16,"kind":"ready"}') {
          child.stdin.write('{"version":16,"kind":"hello"}\n')
          child.stdin.write('{"version":16,"kind":"config","scale":1,"reducedMotion":false,"physicsEnabled":false,"physicsBouncePercent":65,"petPlacement":"center","dialoguePlacement":"near-pet","dialogueWidth":320,"dialogueHeight":420,"randomChatEnabled":false,"randomChatBrowseOnOpen":false,"randomChatConfigured":false,"randomChatMinIntervalMinutes":8,"randomChatMaxIntervalMinutes":24,"randomChatCustomPrompts":[]}\n')
          child.stdin.write('{"version":16,"kind":"state","state":"active","activities":["thinking","working"],"label":"思考中/工作中","sequence":1}\n')
          child.stdin.write('{"version":16,"kind":"state","state":"question","activities":[],"label":"点击回到 Harness 回答","sequence":2}\n')
          setTimeout(() => {
            shutdownSent = true
            child.stdin.write('{"version":16,"kind":"shutdown"}\n')
          }, 250)
