...
test('encodes a v6 canonical outputting active state presentation', () => {
  assert.equal(
    encodeHostMessage({ kind: 'state', state: 'active', activities: ['responding'], label: '输出中…', sequence: 42 }),
    '{"version":6,"kind":"state","state":"active","activities":["responding"],"label":"输出中…","sequence":42}\n',
  )
})

test('encodes a v6 question state with its fixed label', () => {
  assert.equal(
    encodeHostMessage({ kind: 'state', state: 'question', activities: [], label: '等你回答…', sequence: 42 }),
    '{"version":6,"kind":"state","state":"question","activities":[],"label":"等你回答…","sequence":42}\n',
  )
})
...
test('rejects a mixed outputting activity presentation', () => {
  assert.throws(
    () => parseHostMessage('{"version":6,"kind":"state","state":"active","activities":["thinking","responding"],"label":"输出中…","sequence":1}'),
    /activities/,
  )
})
...
test('rejects a question state with a mismatched label', () => {
  assert.throws(
    () => parseHostMessage('{"version":6,"kind":"state","state":"question","activities":[],"label":"等待你的操作","sequence":1}'),
    /label/,
  )
})
