...
test('accepts a v14 ready message', () => {
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"ready"}'),
    { version: 14, kind: 'ready' },
  )
})

test('accepts the payload-free user close request and rejects extra fields', () => {
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"close-requested"}'),
    { version: 14, kind: 'close-requested' },
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"close-requested","text":"ignored"}'),
    /fields/,
  )
})
...
test('accepts the payload-free request to open the already-running Harness and rejects extra fields', () => {
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"open-harness"}'),
    { version: 14, kind: 'open-harness' },
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"open-harness","url":"http://127.0.0.1:1234"}'),
    /fields/,
  )
})

test('accepts the v14 default-layout config and rejects incomplete layout values', () => {
  const config = {
    kind: 'config',
    scale: 1.25,
    reducedMotion: true,
  ...
  assert.equal(line, '{"version":14,"kind":"config","scale":1.25,"reducedMotion":true,"petPlacement":"bottom-right","dialoguePlacement":"near-pet","dialogueWidth":320,"dialogueHeight":420,"randomChatEnabled":false,"randomChatBrowseOnOpen":false,"randomChatConfigured":false,"randomChatMinIntervalMinutes":8,"randomChatMaxIntervalMinutes":24,"randomChatCustomPrompts":[]}\n')
  assert.deepEqual(parseHostMessage(line), { version: 14, ...config })
...
test('encodes a v14 canonical composite active state presentation', () => {
  assert.equal(
    encodeHostMessage({ kind: 'state', state: 'active', activities: ['thinking', 'working'], label: '思考中/工作中', sequence: 42 }),
    '{"version":14,"kind":"state","state":"active","activities":["thinking","working"],"label":"思考中/工作中","sequence":42}\n',
  )
})

test('encodes a v14 canonical outputting active state presentation', () => {
  assert.equal(
    encodeHostMessage({ kind: 'state', state: 'active', activities: ['responding'], label: '输出中…', sequence: 42 }),
    '{"version":14,"kind":"state","state":"active","activities":["responding"],"label":"输出中…","sequence":42}\n',
  )
})

test('encodes a v14 question state with its fixed label', () => {
  assert.equal(
    encodeHostMessage({ kind: 'state', state: 'question', activities: [], label: '点击回到 Harness 回答', sequence: 42 }),
    '{"version":14,"kind":"state","state":"question","activities":[],"label":"点击回到 Harness 回答","sequence":42}\n',
  )
})

test('accepts a bounded helper input and encodes bounded dialogue messages', () => {
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"input","requestId":7,"text":"hello"}'),
    { version: 14, kind: 'input', requestId: 7, text: 'hello' },
  )
  assert.equal(
    ...
    '{"version":14,"kind":"conversation-config","previewEnabled":true,"previewMaxChars":480,"defaultSessionId":null,"defaultWorkspaceId":null}\n',
  )
  assert.equal(
    encodeHostMessage({ kind: 'input-status', requestId: 7, status: 'queued' }),
    '{"version":14,"kind":"input-status","requestId":7,"status":"queued"}\n',
  )
  assert.equal(
    encodeHostMessage({ kind: 'reply-preview', requestId: 7, text: 'ok', completed: false }),
    '{"version":14,"kind":"reply-preview","requestId":7,"text":"ok","completed":false}\n',
  )
  assert.equal(
    encodeHostMessage({ kind: 'clear-preview', requestId: 7, reason: 'next-input' }),
    '{"version":14,"kind":"clear-preview","requestId":7,"reason":"next-input"}\n',
  )
})

test('accepts an input with attachments and an empty text', () => {
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"input","requestId":7,"text":"","attachments":[{"type":"image","mediaType":"image/png","base64":"AAAA"},{"type":"file","path":"C:\\\\a.txt","name":"a.txt"}]}'),
    {
      version: 14,
      kind: 'input',
      requestId: 7,
  ...
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"input","requestId":8,"text":"看图","attachments":[{"type":"image","mediaType":"image/jpeg","base64":"BBBB","name":"pic.jpg"}]}'),
    {
      version: 14,
      kind: 'input',
      requestId: 8,
...
test('round-trips a stop helper message', () => {
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"stop","requestId":12}\n'),
    { version: 14, kind: 'stop', requestId: 12 },
  )
})

test('round-trips content-free one-shot approval messages', () => {
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"approval-answer","requestId":12,"outcome":"allowed-once"}'),
    { version: 14, kind: 'approval-answer', requestId: 12, outcome: 'allowed-once' },
  )
  assert.deepEqual(
    parseHostMessage(encodeHostMessage({ kind: 'approval-request', requestId: 12 })),
    { version: 14, kind: 'approval-request', requestId: 12 },
  )
  assert.deepEqual(
    parseHostMessage(encodeHostMessage({ kind: 'approval-resolved', requestId: 12, outcome: 'cancelled' })),
    { version: 14, kind: 'approval-resolved', requestId: 12, outcome: 'cancelled' },
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"approval-answer","requestId":12,"outcome":"always"}'),
    /approval outcome/,
  )
  assert.throws(
    () => parseHostMessage('{"version":14,"kind":"approval-request","requestId":12,"reason":"C:\\\\secret"}'),
    /fields/,
  )
})

test('rejects invalid v14 dialogue payloads', () => {
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"input","requestId":1,"text":"x","extra":true}'),
    /fields/,
  )
  assert.throws(
    () => parseHelperMessage(JSON.stringify({ version: 14, kind: 'input', requestId: 1, text: 'x'.repeat(2001) })),
    /text/,
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"input","requestId":0,"text":"hello"}'),
    /requestId/,
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"input","requestId":1,"text":" hello"}'),
    /text/,
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"input","requestId":1,"text":""}'),
    /requires text or an attachment/,
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"input","requestId":1,"text":"x","attachments":[]}'),
    /attachments/,
  )
  assert.throws(
    () => parseHelperMessage(`{"version":14,"kind":"input","requestId":1,"text":"x","attachments":[{"type":"image","mediaType":"image/gif","base64":"${'A'.repeat(3000001)}"}]}`),
    /image/,
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"input","requestId":1,"text":"x","attachments":[{"type":"image","mediaType":"image/tiff","base64":"AAAA"}]}'),
    /image/,
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"input","requestId":1,"text":"x","attachments":[{"type":"file","path":""}]}'),
    /file/,
  )
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"stop","requestId":0}'),
    /requestId/,
  )
  assert.throws(
  ...
  assert.throws(
    () => parseHostMessage('{"version":14,"kind":"conversation-config","previewEnabled":true,"previewMaxChars":8001,"defaultSessionId":null,"defaultWorkspaceId":null}'),
    /previewMaxChars/,
  )
  assert.throws(
...
test('parseHostMessage rejects extra fields for every v14 dialogue message kind', () => {
  const messages = [
    { version: 14, kind: 'conversation-config', previewEnabled: true, previewMaxChars: 480, defaultSessionId: null, defaultWorkspaceId: null },
    { version: 14, kind: 'input-status', requestId: 7, status: 'queued' },
    { version: 14, kind: 'reply-preview', requestId: 7, text: 'ok', completed: false },
...
test('accepts v14 dialogue boundaries and rejects 2001-character text', () => {
  const maximumText = 'x'.repeat(2000)
  const ready = '{"version":14,"kind":"ready"}'

  assert.deepEqual(
    parseHelperMessage(JSON.stringify({ version: 14, kind: 'input', requestId: 1, text: maximumText })),
    { version: 14, kind: 'input', requestId: 1, text: maximumText },
  )
...
test('rejects a state message with a free-form label', () => {
  assert.throws(
    () => parseHostMessage('{"version":14,"kind":"state","state":"active","activities":["working"],"label":"C:\\\\secret","sequence":1}'),
    /label/,
  )
})
...
test('rejects non-canonical composite activities', () => {
  assert.throws(
    () => parseHostMessage('{"version":14,"kind":"state","state":"active","activities":["working","thinking"],"label":"思考中/工作中","sequence":1}'),
    /activities/,
  )
})
...
test('rejects a mixed outputting activity presentation', () => {
  assert.throws(
    () => parseHostMessage('{"version":14,"kind":"state","state":"active","activities":["thinking","responding"],"label":"输出中…","sequence":1}'),
    /activities/,
  )
})
...
test('rejects activities for an exclusive state', () => {
  assert.throws(
    () => parseHostMessage('{"version":14,"kind":"state","state":"waiting","activities":["thinking"],"label":"等待你的操作","sequence":1}'),
    /activities/,
  )
})
...
test('rejects a question state with a mismatched label', () => {
  assert.throws(
    () => parseHostMessage('{"version":14,"kind":"state","state":"question","activities":[],"label":"等待你的操作","sequence":1}'),
    /label/,
  )
})
...
test('rejects an unknown helper message kind', () => {
  assert.throws(
    () => parseHelperMessage('{"version":14,"kind":"secret"}'),
    /kind/,
  )
})
...
test('accepts a large image attachment line', () => {
  ...
  const parsed = parseHelperMessage(JSON.stringify({ version: 14, kind: 'input', requestId: 9, text: '', attachments: [{ type: 'image', mediaType: 'image/png', base64 }] }))
...
test('round-trips a request-history helper message', () => {
  const parsed = parseHelperMessage('{"version":14,"kind":"request-history","requestId":9}\n')
  assert.deepEqual(parsed, { version: 14, kind: 'request-history', requestId: 9 })
})

test('round-trips the constrained random-chat messages', () => {
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"random-chat-open","invitationId":9,"topic":"news"}'),
    { version: 14, kind: 'random-chat-open', invitationId: 9, topic: 'news' },
  )
  assert.deepEqual(parseHelperMessage('{"version":14,"kind":"dialogue-closed"}'), { version: 14, kind: 'dialogue-closed' })
  assert.deepEqual(parseHostMessage(encodeHostMessage({ kind: 'random-chat-ready', invitationId: 9 })), { version: 14, kind: 'random-chat-ready', invitationId: 9 })
  assert.deepEqual(parseHostMessage(encodeHostMessage({ kind: 'random-chat-error', invitationId: 9, reason: 'not-configured' })), { version: 14, kind: 'random-chat-error', invitationId: 9, reason: 'not-configured' })
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"random-chat-open","invitationId":9,"topic":"weather"}'),
    { version: 14, kind: 'random-chat-open', invitationId: 9, topic: 'weather' },
  )
  assert.throws(() => parseHelperMessage('{"version":14,"kind":"random-chat-open","invitationId":9,"topic":"location"}'), /topic/)
})

test('encodes the local-only random-chat test command without a payload', () => {
  assert.equal(encodeHostMessage({ kind: 'random-chat-test' }), '{"version":14,"kind":"random-chat-test"}\n')
  assert.deepEqual(parseHostMessage('{"version":14,"kind":"random-chat-test"}'), { version: 14, kind: 'random-chat-test' })
  assert.throws(() => parseHostMessage('{"version":14,"kind":"random-chat-test","text":"do not send"}'), /unexpected fields/)
})
...
test('round-trips a reply host message with the extended limit', () => {
  ...
  assert.deepEqual(parsed, { version: 14, kind: 'reply', requestId: 3, text, completed: true })
})
...
test('round-trips a conversation-history message with block entries', () => {
  ...
  assert.deepEqual(parsed, { version: 14, kind: 'conversation-history', requestId: 4, available: true, messages })
})
...
test('accepts the terminal input statuses', () => {
  for (const status of ['stopped', 'interrupted', 'failed']) {
    ...
    assert.deepEqual(parseHostMessage(line), { version: 14, kind: 'input-status', requestId: 3, status })
  }
})

test('round-trips target-open and every target-answer variant', () => {
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"target-open","requestId":9}'),
    { version: 14, kind: 'target-open', requestId: 9 },
  )
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"target-answer","requestId":9,"sessionId":"s-1","workspaceId":"w-1","newBlank":false}'),
    { version: 14, kind: 'target-answer', requestId: 9, sessionId: 's-1', workspaceId: 'w-1', newBlank: false },
  )
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"target-answer","requestId":9,"sessionId":null,"workspaceId":"w-1","newBlank":true}'),
    { version: 14, kind: 'target-answer', requestId: 9, sessionId: null, workspaceId: 'w-1', newBlank: true },
  )
  assert.deepEqual(
    parseHelperMessage('{"version":14,"kind":"target-answer","requestId":9,"sessionId":null,"workspaceId":null,"newBlank":false,"path":"C:\\\\dir","newWorkspace":true}'),
    { version: 14, kind: 'target-answer', requestId: 9, sessionId: null, workspaceId: null, newBlank: false, path: 'C:\\dir', newWorkspace: true },
  )
})

test('rejects malformed target messages', () => {
  assert.throws(() => parseHelperMessage('{"version":14,"kind":"target-answer","requestId":9,"sessionId":"s-1","workspaceId":null,"newBlank":true}'), /new blank/)
  assert.throws(() => parseHelperMessage('{"version":14,"kind":"target-answer","requestId":9,"sessionId":null,"workspaceId":null,"newBlank":false}'), /empty target/)
  assert.throws(() => parseHelperMessage('{"version":14,"kind":"target-answer","requestId":9,"sessionId":null,"workspaceId":null,"newBlank":false,"path":"C:\\\\dir","newWorkspace":true,"extra":1}'), /fields/)
  assert.throws(() => parseHelperMessage('{"version":14,"kind":"target-answer","requestId":9,"sessionId":null,"workspaceId":"w-1","newBlank":false,"newWorkspace":true,"path":"C:\\\\dir"}'), /invalid workspace create/)
  assert.throws(() => parseHelperMessage('{"version":14,"kind":"target-answer","requestId":9,"sessionId":"s-1","workspaceId":null,"newBlank":false,"path":"C:\\\\dir"}'), /unexpected path/)
})
