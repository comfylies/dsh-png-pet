...
public sealed class ProtocolReaderTests
{
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"shutdown\"}");
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"state\",\"state\":\"active\",\"activities\":[\"thinking\",\"working\"],\"label\":\"思考中/工作中\",\"sequence\":4}");
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"state\",\"state\":\"active\",\"activities\":[\"responding\"],\"label\":\"输出中…\",\"sequence\":4}");
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"state\",\"state\":\"question\",\"activities\":[],\"label\":\"点击回到 Harness 回答\",\"sequence\":4}");

        var state = Assert.IsType<StateMessage>(message);
        Assert.Equal("question", state.State);
        Assert.Empty(state.Activities);
        Assert.Equal("点击回到 Harness 回答", state.Label);
        Assert.Equal(4, state.Sequence);
    }
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"state\",\"state\":\"active\",\"activities\":[\"thinking\"],\"label\":\"思考中…\",\"sequence\":4}");
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"state\",\"state\":\"active\",\"activities\":[\"thinking\"],\"label\":\"思考中…\",\"sequence\":9007199254740991}");
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"config\",\"scale\":1.25,\"reducedMotion\":true,\"physicsEnabled\":true,\"physicsBouncePercent\":50,\"petPlacement\":\"top-center\",\"dialoguePlacement\":\"middle-right\",\"dialogueWidth\":320,\"dialogueHeight\":420,\"dialogueFontSize\":16,\"randomChatEnabled\":false,\"randomChatBrowseOnOpen\":false,\"randomChatConfigured\":false,\"randomChatMinIntervalMinutes\":8,\"randomChatMaxIntervalMinutes\":24,\"randomChatCustomPrompts\":[\"和我聊聊？\"]}");

        var config = Assert.IsType<ConfigMessage>(message);
        Assert.Equal(1.25d, config.Scale);
        Assert.True(config.ReducedMotion);
        Assert.True(config.PhysicsEnabled);
        Assert.Equal(50, config.PhysicsBouncePercent);
        Assert.Equal("top-center", config.PetPlacement);
        Assert.Equal("middle-right", config.DialoguePlacement);
        Assert.Equal(320, config.DialogueWidth);
        Assert.Equal(420, config.DialogueHeight);
        Assert.Equal(16, config.DialogueFontSize);
        Assert.Single(config.RandomChatCustomPrompts);
        Assert.Equal("和我聊聊？", config.RandomChatCustomPrompts[0]);
    }

    [Fact]
    public void Parses_content_free_approval_messages()
    {
        var request = ProtocolReader.Parse("{\"version\":18,\"kind\":\"approval-request\",\"requestId\":7,\"answerable\":true}");
        var resolved = ProtocolReader.Parse("{\"version\":18,\"kind\":\"approval-resolved\",\"requestId\":7,\"outcome\":\"allowed-once\"}");

        Assert.Equal(new ApprovalRequestMessage(7), request);
        Assert.Equal(new ApprovalResolvedMessage(7, "allowed-once"), resolved);
    }

    [Fact]
    public void Rejects_approval_messages_with_payload_or_invalid_outcome()
    {
        Assert.Null(ProtocolReader.Parse("{\"version\":18,\"kind\":\"approval-request\",\"requestId\":7,\"reason\":\"C:\\\\secret\"}"));
        Assert.Null(ProtocolReader.Parse("{\"version\":18,\"kind\":\"approval-resolved\",\"requestId\":7,\"outcome\":\"always\"}"));
    }

    [Fact]
    public void Parses_v7_config_with_default_layout()
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"config\",\"scale\":1.25,\"reducedMotion\":true,\"physicsEnabled\":true,\"physicsBouncePercent\":50,\"petPlacement\":\"bottom-right\",\"dialoguePlacement\":\"near-pet\",\"dialogueWidth\":320,\"dialogueHeight\":420,\"dialogueFontSize\":14,\"randomChatEnabled\":false,\"randomChatBrowseOnOpen\":false,\"randomChatConfigured\":false,\"randomChatMinIntervalMinutes\":8,\"randomChatMaxIntervalMinutes\":24,\"randomChatCustomPrompts\":[]}");
    ...
    [Fact]
    public void Rejects_config_without_the_physics_fields_or_with_an_invalid_bounce_percent()
    {
        Assert.Null(ProtocolReader.Parse("{\"version\":18,\"kind\":\"config\",\"scale\":1,\"reducedMotion\":false,\"petPlacement\":\"center\",\"dialoguePlacement\":\"near-pet\",\"dialogueWidth\":320,\"dialogueHeight\":420,\"randomChatEnabled\":false,\"randomChatBrowseOnOpen\":false,\"randomChatConfigured\":false,\"randomChatMinIntervalMinutes\":8,\"randomChatMaxIntervalMinutes\":24,\"randomChatCustomPrompts\":[]}"));
        Assert.Null(ProtocolReader.Parse("{\"version\":18,\"kind\":\"config\",\"scale\":1,\"reducedMotion\":false,\"physicsEnabled\":true,\"physicsBouncePercent\":101,\"petPlacement\":\"center\",\"dialoguePlacement\":\"near-pet\",\"dialogueWidth\":320,\"dialogueHeight\":420,\"randomChatEnabled\":false,\"randomChatBrowseOnOpen\":false,\"randomChatConfigured\":false,\"randomChatMinIntervalMinutes\":8,\"randomChatMaxIntervalMinutes\":24,\"randomChatCustomPrompts\":[]}"));
    }
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"reply-preview\",\"requestId\":7,\"text\":\"abcdef\",\"completed\":false}");
    ...
    public void Parses_an_extended_reply_preview_text()
    {
        ...
        var message = ProtocolReader.Parse($"{{\"version\":18,\"kind\":\"reply-preview\",\"requestId\":7,\"text\":\"{text}\",\"completed\":false}}");

        Assert.Equal(new ReplyPreviewMessage(7, text, false), message);
        Assert.Null(ProtocolReader.Parse($"{{\"version\":18,\"kind\":\"reply-preview\",\"requestId\":7,\"text\":\"{text}a\",\"completed\":false}}"));
    }

    [Theory]
    [InlineData("{\"version\":18,\"kind\":\"conversation-config\",\"previewEnabled\":true,\"previewMaxChars\":80,\"defaultSessionId\":null,\"defaultWorkspaceId\":null}", true, 80)]
    [InlineData("{\"version\":18,\"kind\":\"conversation-config\",\"previewEnabled\":false,\"previewMaxChars\":8000,\"defaultSessionId\":null,\"defaultWorkspaceId\":null}", false, 8000)]
    ...
    {
        var message = ProtocolReader.Parse($"{{\"version\":18,\"kind\":\"input-status\",\"requestId\":7,\"status\":\"{status}\"}}");
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"clear-preview\",\"requestId\":7,\"reason\":\"next-input\"}");
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"reply\",\"requestId\":7,\"text\":\"final\",\"completed\":true}");
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"conversation-history\",\"requestId\":8,\"available\":true,\"messages\":["
    ...
    {
        var line = "{\"version\":18,\"kind\":\"target-request\",\"requestId\":7,"
    ...
    {
        var message = ProtocolReader.Parse("{\"version\":18,\"kind\":\"target-request\",\"requestId\":7,"
