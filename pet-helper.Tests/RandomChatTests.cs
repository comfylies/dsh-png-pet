...
public sealed class RandomChatTests
{
    ...
    public void Reader_accepts_only_the_constrained_random_chat_protocol_messages()
    {
        Assert.Equal(
            ...
            ProtocolReader.Parse("{\"version\":18,\"kind\":\"random-chat-ready\",\"invitationId\":9}"));
        Assert.Equal(
            new RandomChatErrorMessage(9, "not-configured"),
            ProtocolReader.Parse("{\"version\":18,\"kind\":\"random-chat-error\",\"invitationId\":9,\"reason\":\"not-configured\"}"));
        Assert.Equal(
            new RandomChatTestMessage(),
            ProtocolReader.Parse("{\"version\":18,\"kind\":\"random-chat-test\"}"));
        Assert.Null(ProtocolReader.Parse("{\"version\":18,\"kind\":\"random-chat-error\",\"invitationId\":9,\"reason\":\"free-form\"}"));
    }
