...
- 当前包版本为 dsh-png-pet@0.1.31，协议版本为 v6。安装状态不是源码事实；需要时用 `dsh plugin --profile web list` 确认。已运行的 Harness 必须重启才能加载更新。
...
- 已实现：DSH Web“桌宠”设置（默认会话、实时流式回复开关及流式长度）、桌宠输入发送、回复显示/预览，以及按需读取并显示有限的对话历史。
- 已实现：目标选择卡（右键菜单/拖拽文件夹打开；工作区 → 会话两级选择、未分组桶、空白会话复用、+ 新对话、新建工作区三种入口；`defaultSessionId` 唯一事实源，`defaultWorkspaceId` 为派生提示）。
- 已实现：会话框消息列表模型（决策 B）——IM 式 user/assistant 同屏、按会话自动加载对应历史（选中会话即载入）、流式增量上屏（60ms 节流 + 上滚暂停自动滚动）、Markdig 完成态 markdown 渲染（代码块单独复制、整条复制）、发送中/思考中/生成中状态与桌宠气泡同源、停止键（DSH `agent.cancel` + turn/end 收尾映射 aborted/interrupted/failed，无文本不空白）、图片/文件附件（选择或拖拽；图片 base64 上传 `ctx.attachments.saveImage`，文件路径进提示词）、历史含图片占位与工具调用占位。
