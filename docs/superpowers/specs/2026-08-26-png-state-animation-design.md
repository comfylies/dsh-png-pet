...
每个包含帧的动作还必须声明 `statusAnchor`，其 `x`、`y` 是 0–1 范围内、相对于动作统一透明画布的归一化坐标，表示角色头顶的中点。Helper 用解析后的有效动作（包括回退动作）的锚点将状态气泡放在头顶上方，不使用状态相关的固定像素 `Margin`。窗口在角色图像上方保留透明安全区，避免气泡遮住头部。

同一动作的所有帧必须使用相同尺寸画布，并以脚底作为统一基线。`statusAnchor` 只根据该动作第一帧的头顶位置测量一次；Helper 在动作帧切换时不会重算或移动气泡，因此呼吸、眨眼和轻微上下浮动不会造成气泡抖动。新增坐姿、趴姿或跳跃动作时，素材作者只需为该动作第一帧设置符合新头部位置的 `statusAnchor`，不需要修改协议、Host 或 WPF 布局。
...
- `pet-helper/PetAnimationManifest.cs`：解析、验证动作的帧、头顶锚点和回退链的纯模型；不依赖 WPF 控件或协议读取器。
