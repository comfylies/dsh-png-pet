...
## 请求批准与问卷（本地构建 v0.2.13）

在 DSH 设置 → 桌宠 →「请求批准的位置」选择：

- **Web（默认）**：在原生 Web 回答，桌宠保留状态提示。
- **桌宠**：当前选中会话的受限批准与兼容问卷可直接在桌宠处理。问卷支持最多 8 题、每题最多 8 个选项、单选/多选和最多 1000 字的自定义回答；分页保留本次草稿，提交后等待 Host 确认。
- **Web + 桌宠**：两处同步显示，桌宠卡只读，最终在 Web 回答；可点击「打开 DSH」。本版本尚不支持两个界面同时抢答。

问卷只使用 DSH 正式提问接口的用户可见字段，内容和草稿不保存。超限、计划审阅或带扩展说明的题目保留在 Web；取消、会话切换、超时或 Helper 关闭会清理桌宠请求。协议为 v18，安装后需重启 DSH。
